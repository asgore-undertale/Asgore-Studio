...
OptionsWindow.setFixedSize(OptionsWindow_Width, 480)
...
def cell_bytes():
    ...
    if '[b]' in page_command.toPlainText(): cell_bytes._page_command = bytearray.fromhex(page_command.toPlainText().replace('[b]', '')).decode()
    else: cell_bytes._page_command = page_command.toPlainText()
    if '[b]' in line_command.toPlainText(): cell_bytes._line_command = bytearray.fromhex(line_command.toPlainText().replace('[b]', '')).decode()
    else: cell_bytes._line_command = line_command.toPlainText()
    if '[b]' in textzone_width.toPlainText(): cell_bytes._textzone_width = bytearray.fromhex(textzone_width.toPlainText().replace('[b]', '')).decode()
    else: cell_bytes._textzone_width = textzone_width.toPlainText()
    if '[b]' in textzone_lines.toPlainText(): cell_bytes._textzone_lines = bytearray.fromhex(textzone_lines.toPlainText().replace('[b]', '')).decode()
    else: cell_bytes._textzone_lines = textzone_lines.toPlainText()
    ...
    if Slash_check.isChecked():
        ...
        cell_bytes._page_command = cell_bytes._page_command.replace(r'\n', '\n').replace(r'\t', '\t').replace(r'\r', '\r').replace(r'\a', '\a')
        cell_bytes._line_command = cell_bytes._line_command.replace(r'\n', '\n').replace(r'\t', '\t').replace(r'\r', '\r').replace(r'\a', '\a')
...
def open_def(num):
    if num == 0:
        ...
        fileName, _ = QFileDialog.getOpenFileName(OptionsWindow, 'قاعدة بيانات عرض الحروف', '' , '*.ate')
        if path.exists(fileName) and fileName != '/':
            global chars_width_database_directory, fit_database
            chars_width_database_directory = fileName
            fit_database = Take_From_Table(fileName)
            QMessageBox.about(OptionsWindow, "!!تهانيّ", "تم اختيار قاعدة البيانات.")
    elif num == 3:
...
def convert(text):
    ...
    if FIB_check.isChecked() and not path.exists(chars_width_database_directory):
        QMessageBox.about(CMainWindow, "!!خطأ", "قاعدة بيانات عرض الحروف غير موجودة")
    ...
    if RA_check.isChecked() or C_check.isChecked() or FIB_check.isChecked(): text = Un_Freeze(text)#Freeze Arabic
